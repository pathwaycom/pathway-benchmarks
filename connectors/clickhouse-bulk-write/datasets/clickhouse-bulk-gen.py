"""Generate a sharded CSV dataset for the ClickHouse bulk-write benchmark.
...
Pathway run can read them in parallel (the filesystem reader assigns each shard
to one worker by a stable hash of its path). The directory is mounted read-only
into the Pathway container.

The schema is intentionally identical to the QuestDB bulk-write benchmark so the
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="clickhouse bulk-write dataset generator"
    )
