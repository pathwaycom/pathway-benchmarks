"""Run the ClickHouse bulk-write benchmark across Pathway worker counts.

Two containers (ClickHouse + Pathway). The dataset is generated as CSV shards;
each measured run reads the shard directory (bounded, ``mode="static"``) and
writes every row to ClickHouse over the native protocol. The sharded input lets
the filesystem reader parallelize the read across workers (each shard is owned by
...
This benchmark is deliberately the QuestDB bulk-write benchmark with QuestDB
swapped for ClickHouse: same dataset, same core pinning, same end-to-end timing
and out-of-band correctness verification.

Usage:
    python run_clickhouse_bulk_write.py                       # workers 1 2 4 8
    python run_clickhouse_bulk_write.py --workers 8 --reps 1  # quick check
"""
...
PROJECT = (os.environ.get("USER", "chbulk") + "_chbulk").replace(".", "_")
COMPOSE = [
    "docker",
    "compose",
    "--env-file",
...
def ch_query(sql: str) -> str:
    """Run a SQL statement over the ClickHouse HTTP interface and return the raw
    TabSeparated text. Used only for setup/verification, never inside the timed
    window."""
    port = _env("CLICKHOUSE_HTTP_PORT", "28123")
    user = _env("CLICKHOUSE_USER", "pathway")
    password = _env("CLICKHOUSE_PASSWORD", "pathway")
    resp = requests.post(
...
def ch_count() -> int:
    try:
        return ch_scalar_int(f"SELECT count() FROM {TABLE}")
    except Exception:
        return -1


def verify_correctness(rows: int) -> tuple[bool, dict]:
    """Check, over the ClickHouse HTTP endpoint, that every row landed intact.
    ...
    ``name`` column, the absence of nulls, and — specific to the ClickHouse
    stream-of-changes format — that every appended row is an insertion
    (``diff = 1``), so ``sum(diff)`` must equal the row count.
    """
    expected_sum = rows * (rows - 1) // 2
    checks = {
        "count": ch_scalar_int(f"SELECT count() FROM {TABLE}"),
        "min_k": ch_scalar_int(f"SELECT min(k) FROM {TABLE}"),
        "max_k": ch_scalar_int(f"SELECT max(k) FROM {TABLE}"),
        "sum_k": ch_scalar_int(f"SELECT sum(k) FROM {TABLE}"),
        "sum_diff": ch_scalar_int(f"SELECT sum(diff) FROM {TABLE}"),
        "bad_name": ch_scalar_int(
            f"SELECT count() FROM {TABLE} WHERE name != concat('item_', toString(k))"
        ),
        "null_cols": ch_scalar_int(
...
def gen_dataset(rows: int, shards: int) -> None:
    ...
    sh(
        [
            "python3",
            "datasets/clickhouse-bulk-gen.py",
            "--rows",
            str(rows),
            "--shards",
            str(shards),
...
def run_measured(rows: int, workers: int) -> tuple[float, bool]:
    ...
    # wall-clock of pw.run() reported by the container). ClickHouse native-protocol
    # inserts are synchronous, but give the server a brief grace window before the
    # integrity checks in case parts are still being made visible.
    deadline = time.monotonic() + 60
    while ch_count() < rows and time.monotonic() < deadline:
        time.sleep(0.5)
...
def main():
    ...
    if not os.environ.get("PATHWAY_LICENSE_KEY"):
        raise SystemExit(
            "PATHWAY_LICENSE_KEY is not set. The ClickHouse output connector is a "
    ...
    sh(COMPOSE + ["up", "-d", "--wait", "clickhouse"])
